...
from networks_distill import SConditionGenerator, VGGLoss, load_checkpoint, save_checkpoint, make_grid
from network_generator_distill import SSPADEGenerator,TSPADEGenerator, MultiscaleDiscriminator, GANLoss
...
def train(opt, train_loader, test_loader, test_vis_loader, board, tocg, generator,Tgenerator,discriminator, model):
    """
        Train Generator
    """
    ...
    if opt.fp16:
        if not opt.GT:
            from apex import amp
            [tocg, generator], [optimizer_gen] = amp.initialize(
                [tocg, generator], [optimizer_gen], opt_level='O1', num_losses=2)
        else:
            from apex import amp
            [generator], [optimizer_gen] = amp.initialize(
                [generator], [optimizer_gen], opt_level='O1', num_losses=2)
    ...
    for step in tqdm(range(opt.load_step, opt.keep_step + opt.decay_step)):
        ...
        #                                              Train the generator
        # --------------------------------------------------------------------------------------------------------------
        # print("inputsize: ", torch.cat(agnostic, pose, warped_cloth_paired), dim=1).size())
   
        output_paired = generator(torch.cat((agnostic, pose, warped_cloth_paired), dim=1), parse)
        output_paired_T = Tgenerator(torch.cat((agnostic, pose, warped_cloth_paired), dim=1), parse)
        # print(output_paired.size(),'   ',output_paired_T.size())
        distill_loss= F.mse_loss(output_paired, output_paired_T)
        ...
        G_losses['distill_loss'] = distill_loss
        ...
        if (step + 1) % opt.display_count == 0:
            ...
            print("step: %8d, time: %.3f, G_loss: %.4f, G_adv_loss: %.4f"
                  % (step + 1, t, loss_gen.item(), G_losses['GAN'].mean().item()), flush=True)
...
def main():
    ...
    if not opt.GT:
        ...
        tocg = SConditionGenerator(opt, input1_nc=input1_nc, input2_nc=input2_nc, output_nc=13, ngf=96, norm_layer=nn.BatchNorm2d)
        # Load Checkpoint
        load_checkpoint(tocg, opt.tocg_checkpoint,opt)

    # Generator model
    Sgenerator = SSPADEGenerator(opt, 3+3+3)
    Tgenerator = TSPADEGenerator(opt, 3+3+3)

    Sgenerator.print_network()
    if len(opt.gpu_ids) > 0:
    ...
    Tdiscriminator.load_state_dict(torch.load(opt.Tdis_checkpoint), strict=False)
    # print(Tdiscriminator,' ',type(Tdiscriminator))
    # print(Tdiscriminator.unexpected_keys, ' ',Tdiscriminator.missing_keys )
    # exit(0)
    # Tdiscriminator.cuda()
    
    # Tgenerator = load_checkpoint(Tgenerator, opt.Tgen_checkpoint,opt) 
    Tgenerator.load_state_dict(torch.load(opt.Tgen_checkpoint), strict=False)
    Tgenerator.cuda()
    ...
    if not opt.gen_checkpoint == '' and os.path.exists(opt.gen_checkpoint):
        load_checkpoint(Sgenerator, opt.gen_checkpoint,opt)
        load_checkpoint(Tdiscriminator, opt.dis_checkpoint,opt)

    # Train
    train(opt, train_loader, test_loader, test_vis_loader, board, tocg, Sgenerator,Tgenerator,Tdiscriminator, model)

    # Save Checkpoint
    save_checkpoint(Sgenerator, os.path.join(opt.checkpoint_dir, opt.name, 'gen_model_final.pth'),opt)
    # save_checkpoint(Tdiscriminator, os.path.join(opt.checkpoint_dir, opt.name, 'dis_model_final.pth'),opt)
