...
class SConditionGenerator(nn.Module):
    def __init__(self, opt, input1_nc, input2_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d):
        ...
        self.ClothEncoder = nn.Sequential(
            ...
            ResBlock(ngf * 2, ngf * 4, norm_layer=norm_layer, scale='down'),  # 16
        )
        
        self.PoseEncoder = nn.Sequential(
            ResBlock(input2_nc, ngf, norm_layer=norm_layer, scale='down'),  # 128
            ResBlock(ngf, ngf * 2, norm_layer=norm_layer, scale='down'),  # 64
        ...
        if opt.warp_feature == 'T1':
            ...
            self.SegDecoder = nn.Sequential(
                ResBlock(ngf * 4 * 1 + ngf * 4 , ngf * 4, norm_layer=norm_layer, scale='up'),  # 64
                ResBlock(ngf * 4 * 2 + ngf * 2 , ngf, norm_layer=norm_layer, scale='up'),  # 128
                ResBlock(ngf * 2 * 2 + ngf * 2 , ngf, norm_layer=norm_layer, scale='up')  # 256
            )
        ...
        self.bottleneck = nn.Sequential(
            ...
            nn.Sequential(nn.Conv2d(ngf * 1, ngf * 4, kernel_size=3, stride=1, padding=1, bias=True), nn.ReLU()),
        ...
        self.SegDecoder_inputch=[768,768,567]
    ...
    def forward(self,opt,input1, input2, upsample='bilinear'):
        ...
        # Compute Clothflow
        for i in range(3):
            N, _, iH, iW = E1_list[2 - i].size()
            grid = make_grid(N, iH, iW,opt)

            if i == 0:
                T1 = E1_list[2 - i]  # (ngf * 4) x 8 x 6
                T2 = E2_list[2 - i]
                ...
                #上采样+element-wise加法
                T1 = F.interpolate(T1, scale_factor=2, mode=upsample) + self.conv1[2 - i](E1_list[2 - i])
                T2 = F.interpolate(T2, scale_factor=2, mode=upsample) + self.conv2[2 - i](E2_list[2 - i]) 
                ...
                if self.warp_feature == 'T1':
                    x = self.SegDecoder[i](torch.cat([x, E2_list[2-i], warped_T1], 1))
                if self.warp_feature == 'encoder':
                    warped_E1 = F.grid_sample(E1_list[2-i], flow_norm + grid, padding_mode='border')
                    x = self.SegDecoder[i](torch.cat([x, E2_list[2-i], warped_E1], 1))
...
class TConditionGenerator(nn.Module):
    def __init__(self, opt, input1_nc, input2_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d):
        super(TConditionGenerator, self).__init__()
        self.warp_feature = opt.warp_feature
        self.out_layer_opt = opt.out_layer
...
def define_D(input_nc, ndf=64, n_layers_D=3, norm='instance', use_sigmoid=False, num_D=2, getIntermFeat=False, gpu_ids=[], Ddownx2=False, Ddropout=False, spectral=False):
    ...
    netD = TMultiscaleDiscriminator(input_nc, ndf, n_layers_D, norm_layer, use_sigmoid, num_D, getIntermFeat, Ddownx2, Ddropout, spectral=spectral)
    print(netD)
    if len(gpu_ids) > 0:
