...
from networks_distill import SConditionGenerator,TConditionGenerator, VGGLoss, GANLoss, load_checkpoint, save_checkpoint, define_D
...
def train(opt, train_loader, test_loader, val_loader, board, tocg, tocgT, DT):
    # Model
    tocgT.cuda()
    tocg.cuda()
    tocg.train()
    ...
    for step in tqdm(range(opt.load_step, opt.keep_step)):
        ...
        flow_listT, fake_segmapT, warped_cloth_pairedT, warped_clothmask_pairedT = tocgT(opt,input1, input2)
        
        # 计算光流的L2损失
        flow_loss = 0.0
        # for student_flow, teacher_flow in zip(flow_list, flow_listT):
        #     flow_loss += F.mse_loss(student_flow, teacher_flow)
        flow_loss += F.mse_loss(flow_list[-1], flow_listT[-1])
        
        # 计算伪分割图的L2损失
        fake_segmap_loss = F.mse_loss(fake_segmap, fake_segmapT)

        # 计算变形后衣物的L2损失
        warped_cloth_loss = F.mse_loss(warped_cloth_paired, warped_cloth_pairedT)

        # 计算变形后衣物掩码的L2损失
        warped_clothmask_loss = F.mse_loss(warped_clothmask_paired, warped_clothmask_pairedT)

        # 总损失是所有单独损失的和
        distill_loss = flow_loss + fake_segmap_loss + warped_cloth_loss + warped_clothmask_loss
        
        
        # warped cloth mask one hot 
        
        warped_cm_onehot = torch.FloatTensor((warped_clothmask_paired.detach().cpu().numpy() > 0.5).astype(float)).cuda()
        warped_cm_onehotT = torch.FloatTensor((warped_clothmask_paired.detach().cpu().numpy() > 0.5).astype(float)).cuda()
        ...
        if opt.clothmask_composition != 'no_composition':
            if opt.clothmask_composition == 'detach':
                ...
                cloth_maskT = torch.ones_like(fake_segmapT.detach())
                cloth_maskT[:, 3:4, :, :] = warped_cm_onehotT
                fake_segmapT = fake_segmapT * cloth_maskT
            ...
            if opt.clothmask_composition == 'warp_grad':
                ...
                cloth_maskT = torch.ones_like(fake_segmapT.detach())
                cloth_maskT[:, 3:4, :, :] = warped_clothmask_pairedT
                fake_segmapT = fake_segmapT * cloth_maskT
        ...
        if opt.occlusion:
            ...
            warped_clothmask_pairedT = remove_overlap(F.softmax(fake_segmapT, dim=1), warped_clothmask_pairedT)
            warped_cloth_pairedT = warped_cloth_pairedT * warped_clothmask_pairedT + torch.ones_like(warped_cloth_pairedT) * (1-warped_clothmask_pairedT)
        ...
        fake_clothmaskT = (torch.argmax(fake_segmapT.detach(), dim=1, keepdim=True) == 3).long()
        misalignT = fake_clothmaskT - warped_cm_onehotT
        misalignT[misalignT < 0.0] = 0.0
        
        # loss warping
        # use teacher loss
        # loss_l1_cloth = criterionL1(warped_clothmask_pairedT, parse_cloth_mask)
        # loss_vgg = criterionVGG(warped_cloth_pairedT, im_c)
        ...
        # loss segmentation
        # generator
        # CE_loss = cross_entropy2d(fake_segmapT, label_onehot.transpose(0, 1)[0].long())
        ...
        if opt.no_GAN_loss:
            ...
            # fake_segmap_softmax = torch.softmax(fake_segmapT, 1)
            fake_segmap_softmax = torch.softmax(fake_segmap, 1)

            pred_segmap = DT(torch.cat((input1.detach(), input2.detach(), fake_segmap_softmax), dim=1))
            
            loss_G_GAN = criterionGAN(pred_segmap, True)
            
            if not opt.G_D_seperate:  
                # discriminator
                # fake_segmap_pred = DT(torch.cat((input1.detach(), input2.detach(), fake_segmap_softmax.detach()),dim=1))
                # real_segmap_pred = DT(torch.cat((input1.detach(), input2.detach(), label),dim=1))
                # loss_D_fake = criterionGAN(fake_segmap_pred, False)
                # loss_D_real = criterionGAN(real_segmap_pred, True)

                # loss sum
                loss_G = (0.1*distill_loss+10 * loss_l1_cloth + loss_vgg +opt.tvlambda * loss_tv) + (CE_loss * opt.CElamda + loss_G_GAN * opt.GANlambda)  # warping + seg_generation
        ...
        if (step + 1) % opt.tensorboard_count == 0:
            ...
            board.add_scalar('distill_loss/G', distill_loss.item(), step + 1)
        ...
        if (step + 1) % opt.display_count == 0:
            ...
            if not opt.no_GAN_loss:
                print("step: %8d, time: %.3f\nloss G: %.4f,distill_loss: %.4f, L1_cloth loss: %.4f, VGG loss: %.4f, TV loss: %.4f CE: %.4f, G GAN: %.4f\n"
                    % (step + 1, t, loss_G.item(),distill_loss.item(), loss_l1_cloth.item(), loss_vgg.item(), loss_tv.item(), CE_loss.item(), loss_G_GAN.item()), flush=True)
...
def main():
    ...
    tocg = SConditionGenerator(opt, input1_nc=4, input2_nc=input2_nc, output_nc=opt.output_nc, ngf=96, norm_layer=nn.BatchNorm2d)
    tocgT =TConditionGenerator(opt, input1_nc=4, input2_nc=input2_nc, output_nc=opt.output_nc, ngf=96, norm_layer=nn.BatchNorm2d)
    
    DT = define_D(input_nc=input1_nc + input2_nc + opt.output_nc, Ddownx2 = opt.Ddownx2, Ddropout = opt.Ddropout, n_layers_D=3, spectral = opt.spectral, num_D = opt.num_D)
    ...
    # load TConditionG
    if opt.TConditionG_ckpt == '':
        print('no TConditionGenerator model')
        exit(0)
    else:
        load_checkpoint(tocgT, opt.TConditionG_ckpt,opt)
        
    # load TConditionD
    if  opt.TConditionD_ckpt == '':
        print('no TConditionDiscriminator model')
        exit(0)
    else:
        load_checkpoint(DT, opt.TConditionD_ckpt,opt)

    # Train
    train(opt, train_loader, val_loader, test_loader, board, tocg, tocgT, DT)
