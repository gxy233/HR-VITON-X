...
class SSPADEGenerator(BaseNetwork):
    def __init__(self, opt, input_nc):
        super(SSPADEGenerator, self).__init__()
        ...
        self.G_middle_0 = SPADEResBlock(opt, nf * 16 + 16, nf * 8, use_mask_norm=False)
        # self.G_middle_1 = SPADEResBlock(opt, nf * 16 + 16, nf * 16, use_mask_norm=False)
        self.G_middle_1 = SPADEResBlock(opt, nf * 8 + 16, nf * 4, use_mask_norm=False)
        self.G_middle_2 = SPADEResBlock(opt, nf * 4 + 16, nf * 1, use_mask_norm=False)
        # self.G_middle_3 = SPADEResBlock(opt, nf *1 ,nf //2, use_mask_norm=False)
        ...
        self.up_half = nn.Upsample(scale_factor=2, mode='nearest')
        self.relu = nn.LeakyReLU(0.2)
        self.tanh = nn.Tanh()
    ...
    def forward(self, x, seg):
        ...
        x = self.head_0(features[0], seg) # r
        x = self.up(x)  #4r
        x = self.G_middle_0(torch.cat((x, features[2]), 1), seg) #4r
        if self.num_upsampling_layers in ['more', 'most']:
            x = self.up(x) #16r
        x = self.G_middle_1(torch.cat((x, features[4]), 1), seg) #16r

        x = self.up(x) #64r
        x = self.G_middle_2(torch.cat((x, features[6]), 1), seg) #64r
        x = self.up_half(x) #256rx
        # x = self.G_middle_3(x, seg) # 256r
...
class TSPADEGenerator(BaseNetwork):
    def __init__(self, opt, input_nc):
        super(TSPADEGenerator, self).__init__()
        self.num_upsampling_layers = opt.num_upsampling_layers
        self.param_opt=opt
        self.sh, self.sw = self.compute_latent_vector_size(opt)
